...
static int x2_camera_write_array( struct i2c_client *client, uint32_t addr, sensor_reg_setting_t *regs )
{
    int i, ret = 0;
    ...
    if ( x2_camera->curr_sensor.chip ) {
        if ( enable ) {
            if ( NULL != x2_camera->curr_sensor.start ) {
                ...
                if ( x2_camera_write_array(client, x2_camera->curr_sensor.chip->slave_addr, x2_camera->curr_sensor.start) ) {
            ...
            if ( NULL != x2_camera->curr_sensor.stop ) {
                ...
                if ( x2_camera_write_array(client, x2_camera->curr_sensor.chip->slave_addr, x2_camera->curr_sensor.stop) ) {
...
static int x2_camera_init( x2_camera_t *x2_camera, uint32_t val )
{
    ...
    if ( NULL != x2_camera->curr_sensor.chip ) {
        ...
        if ( x2_camera_read(client, x2_camera->curr_sensor.chip->slave_addr, x2_camera->curr_sensor.chip->chip_id.reg_addr, &chip_id[0]) ) {
            printk(KERN_ERR "get chip id error\n");
            goto err;
        }
        if ( x2_camera_read(client, x2_camera->curr_sensor.chip->slave_addr, x2_camera->curr_sensor.chip->chip_id.reg_addr + 1, &chip_id[1]) ) {
            printk(KERN_ERR "get chip id error\n");
            goto err;
        }
    ...
    if ( val ) {
        if ( NULL != x2_camera->curr_sensor.init ) {
            ...
            if ( x2_camera_write_array(client, x2_camera->curr_sensor.chip->slave_addr, x2_camera->curr_sensor.init) ) {
    ...
    case X2VIDIOC_I2CREAD:
        {
            ...
            if ( reg.reg_size == 1 ) {
                if ( x2_camera_read_byte(client, reg.i2c_addr, (uint8_t)reg.reg, (uint8_t *)&reg.data) ) {
