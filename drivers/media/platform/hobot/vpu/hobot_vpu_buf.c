...
static avl_node_t *do_unlink(avl_node_t * tree)
{
	...
	if (end_node) {
		...
		end_node->right =
		    (tree->right != end_node) ? tree->right : end_node->right;
		end_node->height =
...
static avl_node_t *avltree_remove(avl_node_t * tree,
				  avl_node_t ** found_node, vmem_key_t key)
{
	*found_node = NULL;
	if (tree == NULL) {
...
static void avltree_free(avl_node_t * tree)
{
	if (tree == NULL)
		return;
...
static avl_node_t *remove_approx_value(avl_node_t * tree,
				       avl_node_t ** found, vmem_key_t key)
{
	*found = NULL;
	if (tree == NULL) {
		return NULL;
...
int vmem_init(video_mm_t * mm, unsigned long addr, unsigned long size)
{
	...
	if (mm->page_list == NULL) {
		printk(KERN_ERR "%s:%d failed to kmalloc(%d)\n", __func__,
		       __LINE__, (int)(mm->num_pages * sizeof(page_t)));
		return -1;
	}
