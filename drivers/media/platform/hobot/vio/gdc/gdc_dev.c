...
static ssize_t x2a_gdc_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
	return 0;
}

static ssize_t x2a_gdc_read(struct file *file, char __user *buf, size_t size, loff_t *ppos)
{
	return 0;
...
void gdc_init(struct x2a_gdc_dev *gdc_dev, gdc_settings_t *gdc_settings)
{
	...
	gdc_set_config_size(base_addr, gdc_settings->gdc_config.config_size/4);
	gdc_set_rdma_img_width(base_addr, gdc_settings->gdc_config.input_width);
	gdc_set_rdma_img_height(base_addr, gdc_settings->gdc_config.input_height);
	gdc_set_wdma_img_width(base_addr, gdc_settings->gdc_config.output_width);
	gdc_set_wdma_img_height(base_addr, gdc_settings->gdc_config.output_height);
	vio_info("config_addr:%x,config_size:%x\n", gdc_settings->gdc_config.config_addr,gdc_settings->gdc_config.config_size);
...
int gdc_process(struct x2a_gdc_dev *gdc_dev, gdc_settings_t *gdc_settings)
{
	...
	if(num_input >= 1) {
		...
		if(gdc_settings->gdc_config.sequential_mode == 1) {
			gdc_set_rdma0_img_addr(base_addr, input_addr[gdc_settings->seq_planes_pos]);
		...
		vio_info("data1in lineoffset:%d, hight:%d\n", lineoffset, height);
	}
	if(num_input >= 2 && gdc_settings->gdc_config.sequential_mode == 0) { //only processed if not in toggle mode
		...
		vio_info("data2in lineoffset:%d, hight:%d\n", lineoffset, height);
	}
	...
	if(num_input >= 1) {
		...
		vio_info("out1: plain_num:%d case, lineoffset:%d, hight:%d\n", num_input, lineoffset, height);
		...
		gdc_set_wdma0_img_addr(base_addr, gdc_settings->Out_buffer_addr[0]);
	...
	if(num_input >= 2 && gdc_settings->gdc_config.sequential_mode == 0){
		lineoffset = gdc_settings->gdc_config.output_stride >> gdc_settings->gdc_config.div_width;
		height = gdc_settings->gdc_config.output_height >> gdc_settings->gdc_config.div_height;
		vio_info("out2: plain_num:%d case, lineoffset:%d, hight:%d\n", num_input, lineoffset, height);
		gdc_set_wdma1_img_addr(base_addr, gdc_settings->Out_buffer_addr[1]);
	...
	if(num_input >= 3 && gdc_settings->gdc_config.sequential_mode == 0){
		lineoffset = gdc_settings->gdc_config.output_width >> gdc_settings->gdc_config.div_width;
		height = gdc_settings->gdc_config.output_height >> gdc_settings->gdc_config.div_height;
		gdc_set_wdma2_img_addr(base_addr, gdc_settings->Out_buffer_addr[1]);
...
int gdc_video_process(struct gdc_video_ctx *gdc_ctx, unsigned long arg)
{
	...
	ret = copy_from_user(&gdc_settings, (gdc_settings_t *)arg, sizeof(gdc_settings_t));
	if(ret){
	...
	down_interruptible(&gdc_dev->smp_gdc_enable);
...
static long x2a_gdc_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	struct gdc_video_ctx *gdc_ctx;
	struct gdc_group *group;
	int ret = 0;
...
int x2a_gdc_device_node_init(struct x2a_gdc_dev *gdc)
{
	...
	dev = device_create(gdc->class, NULL, MKDEV(MAJOR(gdc->devno), 0), NULL, name);
	if (IS_ERR(dev)) {
		ret  = -EINVAL;
...
static int x2a_gdc_probe(struct platform_device *pdev)
{
	...
	gdc->base_reg = devm_ioremap_nocache(&pdev->dev, mem_res->start, resource_size(mem_res));
	if (!gdc->base_reg) {
...
static int __init x2a_gdc_init(void)
{
	...
	sema_init(&gdc->smp_gdc_enable, 1);
	gdc->base_reg = kzalloc(0x1000, GFP_KERNEL);
