...
static ssize_t x2a_pym_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
	return 0;
}

static ssize_t x2a_pym_read(struct file *file, char __user *buf, size_t size, loff_t *ppos)
{
	return 0;
...
static void frame_work_function(struct kthread_work *work){
	...
	if(frame){
		...
		if(!test_bit(PYM_OTF_INPUT, &pym->state)){
			pym_rdma_set_addr(pym->base_reg, frame->frameinfo.addr[0], frame->frameinfo.addr[1]);
...
void pym_update_param(struct pym_video_ctx *pym_ctx, pym_cfg_t *pym_config)
{
	...
	for(i = 0; i < MAX_PYM_DS_COUNT; i++){
		...
		pym_ds_config_factor(pym->base_reg, shadow_index, i, pym_config->stds_box[i].factor);
	...
	for(i = 0; i < MAX_PYM_US_COUNT; i++){
		...
		pym_us_config_factor(pym->base_reg, shadow_index, i, pym_config->stus_box[i].factor);
...
int pym_video_init(struct pym_video_ctx *pym_ctx, unsigned long arg)
{
	...
	if(!(pym_ctx->state & (BIT(VIO_VIDEO_OPEN) | BIT(VIO_VIDEO_REBUFS)))){
		vio_err("[V%02d] invalid INIT is requested(%lX)", group->instance, pym_ctx->state);
		return -EINVAL;
	}

	vio_info("pym_cfg_t length = %d\n", sizeof(pym_cfg_t));
	ret = copy_from_user((char *)&pym_config, (u32 __user *) arg, sizeof(pym_cfg_t));
	if(ret)
		return -EFAULT;

	if(test_bit(PYM_OTF_INPUT, &pym_dev->state) && (pym_config.img_scr != 1)){
...
int pym_video_streamon(struct pym_video_ctx *pym_ctx)
{
	...
	if(!(pym_ctx->state & (BIT(VIO_VIDEO_STOP) | BIT(VIO_VIDEO_REBUFS)))){
		vio_err("[V%02d] invalid STREAM ON is requested(%lX)", pym_ctx->group->instance, pym_ctx->state);
		return -EINVAL;
	}
...
int pym_video_streamoff(struct pym_video_ctx *pym_ctx)
{
	...
	if(!(pym_ctx->state & BIT(VIO_VIDEO_START))){
		vio_err("[V%02d] invalid STREAMOFF is requested(%lX)", pym_ctx->group->instance, pym_ctx->state);
		return -EINVAL;
	}
	pym_dev = pym_ctx->pym_dev;
...
int pym_video_reqbufs(struct pym_video_ctx *pym_ctx, u32 buffers)
{
	...
	if(!(pym_ctx->state & (BIT(VIO_VIDEO_STOP) | BIT(VIO_VIDEO_REBUFS) | BIT(VIO_VIDEO_INIT)))){
		vio_err("[V%02d] invalid REQBUFS is requested(%lX)", pym_ctx->group->instance, pym_ctx->state);
		return -EINVAL;
	}
	...
	for(i = 0; i < buffers; i++){
		...
		//framemgr->frames[i].instance = pym_ctx->group->instance;
	}
...
static long x2a_pym_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	int ret = 0;
	int buffers = 0;
	int enable = 0;
...
int x2a_pym_device_node_init(struct x2a_pym_dev *pym)
{
	...
	dev = device_create(pym->class, NULL, MKDEV(MAJOR(devno), 0), NULL, "pym");
	if (IS_ERR(dev)) {
		ret  = -EINVAL;
...
static int x2a_pym_probe(struct platform_device *pdev)
{
	...
	pym->base_reg = devm_ioremap_nocache(&pdev->dev, mem_res->start, resource_size(mem_res));
	if (!pym->base_reg) {
