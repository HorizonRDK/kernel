...
#if ACAMERA_LOG_REAL_TIME // enable change printk level & mask
...
#define ACAMERA_LOGLIST_SET_LEVEL( level ) acamera_logger_set_level( level )
#define ACAMERA_LOGLIST_SET_MASK( mask ) acamera_logger_set_mask( mask )

#else//disable change printk level & mask

#ifdef ACAMERA_LOG_LEVEL
	#define _ACAMERA_LOG_OUTPUT_LEVEL ACAMERA_LOG_LEVEL
#else
	#define _ACAMERA_LOG_OUTPUT_LEVEL 6 //nothing
#endif
...
#define ACAMERA_LOGLIST_SET_LEVEL( level ) (void)0
#define ACAMERA_LOGLIST_SET_MASK( mask ) (void)0
...
#define ACAMERA_LOG_WRITE(func, file, line, level, module, fmt, arg )              \
    do {                                                          \
        if ( ACAMERA_LOG_ON( level, 1 << module ) )               \
            _acamera_log_write( func, file, line, \
                                level, module, fmt, arg );     \
    } while ( 0 )
#if ACAMERA_LOG_FROM_ISR
#define ACAMERA_LOG_ISR( func, file, line, level, module, fmt, arg )                         \
    do {                                                              \
        if ( ACAMERA_LOG_ON( level, 1 << module ) )                   \
            _acamera_log_write_isr( func, file, line, \
                                    level, module, fmt, arg );     \
    } while ( 0 )
#else
#define ACAMERA_LOG_ISR(func,file, line, level, module, fmt, arg ) (void)0
#endif
#else
#if ACAMERA_LOG_FROM_ISR
...
#define ACAMERA_LOG_WRITE( func, file, line, level, module, fmt, arg )               \
    do {                                                      \
        if ( ACAMERA_LOG_ON( level, 1 << module ) )           \
            _acamera_log_write( level, module, fmt, arg ); \
    } while ( 0 )
#if ACAMERA_LOG_FROM_ISR
#define ACAMERA_LOG_ISR( func, file, line, level, module, fmt, arg )                     \
    do {                                                          \
        if ( ACAMERA_LOG_ON( level, 1 << module ) )               \
            _acamera_log_write_isr( level, module, fmt, arg ); \
    } while ( 0 )
#else
#define ACAMERA_LOG_ISR( func, file, line, level, module, fmt, arg ) (void)0
#endif
...
#define LOGLIST( func, file, line, level, log_module, fmt, arg ) ACAMERA_LOG_WRITE(func, file, line, level, log_module, fmt, arg )
#define LOGLIST_ISR( func, file, line, level, log_module, fmt, arg ) ACAMERA_LOG_ISR( func, file, line, level, log_module, fmt, arg )
...
//#define LOG_MODULE 0 
//always the first module, src file should undef this value
