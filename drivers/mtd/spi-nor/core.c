...
static int spi_nor_check(struct spi_nor *nor)
{
	...
	if (nor->spimem && nor->controller_ops) {
		dev_err(nor->dev, "nor->spimem and nor->controller_ops %s",
					"are mutually exclusive, please set just one of them.\n");
		return -EINVAL;
	}
				 ...
				 const struct spi_nor_hwcaps *hwcaps)
{
	...
	if (err) {
		dev_dbg(nor->dev,
			"can't select read settings supported by %s",
			"both the SPI controller and memory.\n");
		return err;
	}
	...
	if (err) {
		dev_dbg(nor->dev,
			"can't select write settings supported by %s",
			"both the SPI controller and memory.\n");
		return err;
	}
	...
	if (err) {
		dev_dbg(nor->dev,
			"can't select erase settings supported by %s",
			"both the SPI controller and memory.\n");
		return err;
	}
