	...
	if (cmd_action == HostCmd_ACT_GEN_SET) {
		...
		if (net_mon->enable_net_mon && net_mon->channel) {
			...
			if (net_mon->band & BAND_GN) {
				bw_offset = net_mon->sec_chan_offset;
...
/**
 *  @brief This function set the smart mode configuration.
 *
 *  @param pmpriv         A pointer to mlan_private structure
 *  @param pcmd         A pointer to HostCmd_DS_COMMAND structure
 *  @param cmd_action   Command action
 ...
 */
mlan_status
wlan_cmd_smart_mode_cfg(mlan_private *pmpriv,
			HostCmd_DS_COMMAND *pcmd,
			t_u16 cmd_action, void *pdata_buf)
{
	HostCmd_DS_SMART_MODE_CFG *hostcmd_smc = &pcmd->params.smart_mode_cfg;
	mlan_ds_smc *smc = (mlan_ds_smc *) pdata_buf;
	MrvlIEtypes_Multi_addr_range_t *paddr_tlv = MNULL;
	MrvlIEtypes_Frame_Filter_t *pfilter_tlv = MNULL;
	MrvlIEtypes_beacon_period_t *pbcnperiod_tlv = MNULL;
	MrvlIEtypes_beacon_of_channel_t *pbcnofchan_tlv = MNULL;
	MrvlIEtypes_ChanListParamSet_t *pchanlist_tlv = MNULL;
	MrvlIEtypes_SsIdParamSet_t *pssid_tlv = MNULL;
	MrvlIEtypes_Data_t *pie_tlv = MNULL;
	t_u8 *ptlv_buffer = MNULL;
	t_u16 i = 0;
	t_u8 chan2Offset = SEC_CHAN_NONE;

	ENTER();

	pcmd->command = wlan_cpu_to_le16(HostCmd_CMD_SMART_MODE_CFG);
	hostcmd_smc->action = wlan_cpu_to_le16(smc->action);
	pcmd->size = S_DS_GEN + sizeof(hostcmd_smc->action);

	if (smc->action == HostCmd_ACT_GEN_SET) {
		ptlv_buffer = hostcmd_smc->tlv_buffer;

		for (i = 0; i < smc->addrCount && i < MAX_ADDR_FILTER; i++) {
			paddr_tlv =
				(MrvlIEtypes_Multi_addr_range_t *) ptlv_buffer;
			paddr_tlv->header.type =
				wlan_cpu_to_le16(TLV_TYPE_MULTI_ADDR_RANGE);
			paddr_tlv->header.len =
				sizeof(MrvlIEtypes_Multi_addr_range_t) -
				sizeof(MrvlIEtypesHeader_t);
			memcpy(pmpriv->adapter, paddr_tlv->start_addr,
			       smc->addr[i].startaddr, MLAN_MAC_ADDR_LENGTH);
			memcpy(pmpriv->adapter, paddr_tlv->end_addr,
			       smc->addr[i].endaddr, MLAN_MAC_ADDR_LENGTH);
			paddr_tlv->filtertype =
				wlan_cpu_to_le16(smc->addr[i].filterType);
			ptlv_buffer +=
				(paddr_tlv->header.len +
				 sizeof(MrvlIEtypesHeader_t));
			paddr_tlv->header.len =
				wlan_cpu_to_le16(paddr_tlv->header.len);
		}

		if (smc->chan_num) {
			pchanlist_tlv =
				(MrvlIEtypes_ChanListParamSet_t *)ptlv_buffer;
			pchanlist_tlv->header.type =
				wlan_cpu_to_le16(TLV_TYPE_CHANLIST);
			for (i = 0; i < smc->chan_num && i < MAX_CHANNEL_LIST;
			     i++) {
				if (smc->chanlist[i].chan_number < 36)
					pchanlist_tlv->chan_scan_param[i].
						bandcfg.chanBand = BAND_2GHZ;
				else
					pchanlist_tlv->chan_scan_param[i].
						bandcfg.chanBand = BAND_5GHZ;
		/**channel width*/
				pchanlist_tlv->chan_scan_param[i].bandcfg.
					chanWidth = smc->chanlist[i].chanwidth;
		/**second off channel*/
				chan2Offset =
					wlan_get_second_channel_offset(smc->
								       chanlist
								       [i].
								       chan_number);
				pchanlist_tlv->chan_scan_param[i].bandcfg.
					chan2Offset = chan2Offset;

				pchanlist_tlv->chan_scan_param[i].chan_number =
					smc->chanlist[i].chan_number;
				pchanlist_tlv->chan_scan_param[i].
					min_scan_time =
					wlan_cpu_to_le16(smc->chanlist[i].
							 minScanTime);
				pchanlist_tlv->chan_scan_param[i].
					max_scan_time =
					wlan_cpu_to_le16(smc->chanlist[i].
			...
			pchanlist_tlv->header.len =
				sizeof(ChanScanParamSet_t) * i;
			ptlv_buffer +=
				(pchanlist_tlv->header.len +
				 sizeof(MrvlIEtypesHeader_t));
			pchanlist_tlv->header.len =
				wlan_cpu_to_le16(pchanlist_tlv->header.len);
		}

		if (smc->filterCount) {
			pfilter_tlv =
				(MrvlIEtypes_Frame_Filter_t *) ptlv_buffer;
			pfilter_tlv->header.type =
				wlan_cpu_to_le16(TLV_TYPE_FRAME_FILTER);
			for (i = 0;
			     i < smc->filterCount && i < MAX_FRAME_FILTER;
			     i++) {
				pfilter_tlv->type[i].destType =
					smc->framefilter[i].destType;
				pfilter_tlv->type[i].frameType =
					smc->framefilter[i].frameType;
			}
			pfilter_tlv->header.len =
				sizeof(Frame_Filter_Type_t) * i;
			ptlv_buffer +=
				(pfilter_tlv->header.len +
				 sizeof(MrvlIEtypesHeader_t));
			pfilter_tlv->header.len =
				wlan_cpu_to_le16(pfilter_tlv->header.len);
		}
		...
		if (smc->ssid_len) {
			...
			pssid_tlv->header.len = (t_u16)smc->ssid_len;
			memcpy(pmpriv->adapter, pssid_tlv->ssid, smc->ssid,
			       pssid_tlv->header.len);
			ptlv_buffer +=
				sizeof(pssid_tlv->header) +
				pssid_tlv->header.len;
			pssid_tlv->header.len =
...
#ifdef UAP_SUPPORT
			pbcnperiod_tlv =
				(MrvlIEtypes_beacon_period_t *)ptlv_buffer;
			pbcnperiod_tlv->header.type =
				wlan_cpu_to_le16(TLV_TYPE_UAP_BEACON_PERIOD);
			pbcnperiod_tlv->header.len =
				sizeof(MrvlIEtypes_beacon_period_t)
				- sizeof(MrvlIEtypesHeader_t);
			pbcnperiod_tlv->beacon_period =
				wlan_cpu_to_le16(smc->beaconPeriod);
			ptlv_buffer +=
				sizeof(MrvlIEtypesHeader_t) +
				pbcnperiod_tlv->header.len;
			pbcnperiod_tlv->header.len =
				wlan_cpu_to_le16(pbcnperiod_tlv->header.len);
#endif

			pbcnofchan_tlv =
				(MrvlIEtypes_beacon_of_channel_t *) ptlv_buffer;
			pbcnofchan_tlv->header.type =
				wlan_cpu_to_le16(TLV_TYPE_BEACON_OF_CHAN);
			pbcnofchan_tlv->header.len =
				sizeof(MrvlIEtypes_beacon_of_channel_t)
				- sizeof(MrvlIEtypesHeader_t);
			pbcnofchan_tlv->beacon_of_chan =
				smc->beaconOfChan | (smc->probeOfChan << 1);
			ptlv_buffer +=
				sizeof(MrvlIEtypesHeader_t) +
				pbcnofchan_tlv->header.len;
			pbcnofchan_tlv->header.len =
				wlan_cpu_to_le16(pbcnofchan_tlv->header.len);
		}

		if (smc->ie_len) {
			pie_tlv = (MrvlIEtypes_Data_t *)ptlv_buffer;
			pie_tlv->header.type =
				wlan_cpu_to_le16(TLV_TYPE_PASSTHROUGH);
			pie_tlv->header.len = smc->ie_len;
			memcpy(pmpriv->adapter, pie_tlv->data, smc->customer_ie,
			       smc->ie_len);
			ptlv_buffer +=
				(pie_tlv->header.len +
				 sizeof(MrvlIEtypesHeader_t));
			pie_tlv->header.len =
				wlan_cpu_to_le16(pie_tlv->header.len);
		}
		pcmd->size += (t_u16)(ptlv_buffer - hostcmd_smc->tlv_buffer);
	}
			 ...
			 IN t_void *pdata_buf, IN t_void *pcmd_buf)
{
	...
	case HostCmd_CMD_SMART_MODE_CFG:
		ret = wlan_cmd_smart_mode_cfg(pmpriv, cmd_ptr, cmd_action,
					      pdata_buf);
		break;
