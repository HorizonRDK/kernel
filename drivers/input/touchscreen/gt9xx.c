...
static int set_int_as_out(struct i2c_client *client, int val);
static int set_int_as_in(struct i2c_client *client);
...
#if GTP_REVERT_X_FLAG
	x = GTP_MAX_WIDTH - x;
#endif

#if GTP_REVERT_Y_FLAG
	y = GTP_MAX_HEIGHT - y;
#endif
...
#else
		GTP_INFO("Touch to light up the screen!");
		doze_status = DOZE_WAKEUP;
		input_report_key(ts->input_dev, KEY_POWER, 1);
		input_sync(ts->input_dev);
		input_report_key(ts->input_dev, KEY_POWER, 0);
...
void gtp_int_sync(struct i2c_client *client, s32 ms)
{
	...
	set_int_as_out(client, 0);
	msleep(ms);
	set_int_as_in(client);
...
void gtp_reset_guitar(struct i2c_client *client, s32 ms)
{
	...
	set_int_as_out(client, client->addr == 0x14);
	msleep(1);
...
#define cfg_info_group0 CTP_CFG_GROUP0
	...
	if (SUCCESS == ret)
	{
		if (sensor_id >= 0x06)
		{
			...
			sensor_id = 0;
			// ts->pnl_init_error = 1; // qkdang
			// return -1; // qkdang
		}
	}
	else
	{
		GTP_ERROR("Failed to get sensor_id!");
		ts->pnl_init_error = 1;
		return -1;
	}
...
static s8 gtp_i2c_test(struct i2c_client *client)
{
	...
	for(i=0; i<2500; i ++){
		test[2] = 0x5A;
		gtp_i2c_write(client, test, 3);
		if(0 >= gtp_i2c_read(client, test, 3) || test[2] != 0x5A){
			printk("reg test error %d 0x%x\n", i, gtp_i2c_read(client, test, 3));
			break;
		}
		test[2] = 0x15;
		gtp_i2c_write(client, test, 3);
		if(0 >= gtp_i2c_read(client, test, 3) || test[2] != 0x15){
			printk("reg test error %d 0x%x\n", i, gtp_i2c_read(client, test, 3));
			break;
		}
		test[2] = 0x5A;
		...
		test[2] = 0x48;
		gtp_i2c_write(client, test, 3);
		if(0 >= gtp_i2c_read(client, test, 3) || test[2] != 0x48){
			printk("reg test error %d 0x%x\n", i, gtp_i2c_read(client, test, 3));
			break;
		}
	}
...
Output:
	Executive outcomes.
		>= 0: succeed, < 0: failed
*******************************************************/
static s8 gtp_request_io_reset(struct goodix_ts_data *ts)
{
	int ret = 0;
...
static s8 gtp_request_io_irq(struct goodix_ts_data *ts)
{
	s32 ret = 0;
...
int set_int_as_out(struct i2c_client *client, int val)
{
#if 1
	GTP_GPIO_OUTPUT(gtp_int_gpio, val);
#else
...
int set_int_as_in(struct i2c_client *client)
{
#if 1
		GTP_GPIO_AS_INPUT(gtp_int_gpio);
#else
...
#if 0 // no regulator
	if (!vdd_ana) {
		GTP_INFO("GTP Get vcc-ctp");
		vdd_ana = regulator_get(&client->dev, "vcc-ctp");
		if (IS_ERR(vdd_ana)) {
...
#if GTP_GESTURE_WAKEUP
static ssize_t gtp_runtime_suspend_show(struct device *dev,
...
static ssize_t gtp_runtime_suspend_store(struct device *dev,
					 struct device_attribute *attr,
					 const char *buf, size_t count)
{
	...
	if (val > 1)
		...
		doze_status = val ? DOZE_DISABLED : DOZE_ENABLED;

	return count;
}

static DEVICE_ATTR(runtime_suspend, 0664	, gtp_runtime_suspend_show, gtp_runtime_suspend_store);
...
static struct attribute_group gtp_attr_group = {
   .name = "input",
...
#if GTP_ESD_PROTECT
	ts->clk_tick_cnt = 4 * HZ;	  // HZ: clock ticks in 1 second generated by system
	GTP_DEBUG("Clock ticks for an esd cycle: %d", ts->clk_tick_cnt);
	spin_lock_init(&ts->esd_lock);
	// ts->esd_lock = SPIN_LOCK_UNLOCKED;
#endif
	ts->gtp_rawdiff_mode = 0;
	ret = gtp_request_io_irq(ts);
	if (ret < 0)
	{
		GTP_ERROR("GTP request IO port failed.");
		kfree(ts);
...
static void goodix_ts_resume(struct goodix_ts_data *ts)
{
	...
	{
		GTP_DEBUG("no irq, use hrtimer_start");
		hrtimer_start(&ts->timer, ktime_set(1, 0), HRTIMER_MODE_REL);
	}
...
static void gtp_esd_check_func(struct work_struct *work)
{
	...
	if (i >= 3)
	{
...
#endif
		{
			...
			// added by wcliu
			GTP_GPIO_OUTPUT(gtp_rst_gpio, 0);   // begin select I2C slave addr
			GTP_GPIO_SET_VALUE(gtp_rst_gpio, 0);
			msleep(50);                         // T2: > 10ms
			// HIGH: 0x28/0x29, LOW: 0xBA/0xBB
			GTP_GPIO_OUTPUT(gtp_int_gpio, 0);
			GTP_GPIO_SET_VALUE(gtp_int_gpio, 0);
			GTP_DEBUG("sleep 5000ms");
			msleep(5000);

#ifdef GTP_CONFIG_OF
			gtp_power_switch(ts->client, 0);
			GTP_DEBUG("sleep 20ms");
			msleep(100);
			gtp_power_switch(ts->client, 1);
			GTP_DEBUG("sleep 20ms");
...
static struct i2c_board_info gt9xx_info = {
	I2C_BOARD_INFO(GTP_I2C_NAME, GT9XX_ADDR),
};
