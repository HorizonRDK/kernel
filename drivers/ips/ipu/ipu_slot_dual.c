...
int8_t init_ipu_slot_dual(uint64_t base, slot_ddr_info_dual_t * data)
{
	...
	for (i = 0; i < IPU_MAX_SLOT_DUAL; i++) {
		...
		enqueue_slot(&g_dual_slot_queue[FREE_SLOT_QUEUE],
			     &g_ipu_slot_dual[i]);
		...
		ipu_dbg("slot-%d, crop=0x%x\n", i,
			g_ipu_slot_dual[i].info_h.ddr_info.crop.y_offset);
		ipu_dbg("slot-%d, scale=0x%x\n", i,
			g_ipu_slot_dual[i].info_h.ddr_info.scale.y_offset);
		ipu_dbg("ds1st[%d].y_offset=0x%x\n", i,
			g_ipu_slot_dual[i].info_h.ddr_info.ds_1st[i].y_offset);
		ipu_dbg("ds2nd[%d].y_offset=0x%x\n", i,
			g_ipu_slot_dual[i].info_h.ddr_info.ds_2nd[i].y_offset);
	}
	return 0;
}
