...
static int dwc3_get_dr_mode(struct dwc3 *dwc)
{
	...
	if (mode != dwc->dr_mode) {
		if (dwc->dr_mode == USB_DR_MODE_OTG) {
			dev_info(dev,
				"Configuration mismatch. user dr_mode(%s) "
				"not equal with hardware default mode(%s). "
				"still use %s mode\n",
				 usb_role_string(dwc->dr_mode),
				 usb_role_string(mode),
				 usb_role_string(dwc->dr_mode));
		} else {
			dev_warn(dev,
...
{
	if (0 != sysfs_create_group(&dwc->dev->kobj, &dwc3_attr_group))
		dev_err(dwc->dev, "sysfs_create_group failed\n");
}
...
static int dwc3_probe(struct platform_device *pdev)
{
	...
	if (dev->of_node) {
		...
		if (IS_ERR(dwc->ref_clk)) {
			ret = PTR_ERR(dwc->ref_clk);
			if (ret == -EPROBE_DEFER) {
				return ret;
			} else {
				dev_err(dev, "could not get ref clock\n");
				return ret;
			}
		}
		...
		if (IS_ERR(dwc->susp_clk)) {
			ret = PTR_ERR(dwc->susp_clk);
			if (ret == -EPROBE_DEFER) {
				return ret;
			} else {
				dev_err(dev, "could not get suspend clock\n");
				return ret;
			}
		}
