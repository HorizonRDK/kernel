 ...
 */
static void x2_qspi_write(struct x2_qspi *xqspi, u32 offset,
				      u32 val)
{
...
int x2_qspi_cfg_line_mode(struct x2_qspi *xqspi, uint32_t line_mode, bool enable)
{
	uint32_t ret = 0, val;
...
{
	uint32_t val = 0, i;
	for (i = xqspi->regs + QSPI_TX_RX_REG; i <= xqspi->regs + QSPI_XIP_CFG; i = i + 4) {
...
static int x2_qspi_read_data(struct x2_qspi *xqspi, uint8_t *rxbuf, uint32_t len)
{
	int32_t i = 0;
	uint32_t rx_len = 0;
	uint8_t *ptr = rxbuf;
		...
		do {
			...
			while (rx_len > 0) {
				if (rx_len > level) {
					...
					for (i = 0; i < tmp_rxlen; i++)
						x2_qspi_write(xqspi, QSPI_TX_RX_REG, 0x0);
...
static int x2_qspi_flash_read_reg(struct spi_nor *nor, u8 opcode, u8 *buf, int len)
{
	int ret;
				      ...
				      struct spi_transfer *transfer)
{
	...
	if (xqspi->txbuf != NULL) {
		...
		transfer_len = x2_qspi_write_data(xqspi, xqspi->txbuf, transfer->len);
	...
	if (xqspi->rxbuf != NULL) {
		...
		transfer_len = x2_qspi_read_data(xqspi, xqspi->rxbuf, transfer->len);
...
static int x2_qspi_probe(struct platform_device *pdev)
{
	...
	for_each_available_child_of_node(pdev->dev.of_node, nc) {
		...
		ret = of_property_read_u32(nc, "spi-max-frequency", &max_speed_hz);
		if (!ret) {
			break;
		} else {
	...
	master->unprepare_transfer_hardware =
					x2_unprepare_transfer_hardware;
