...
static void parse_opts(int argc, char *argv[])
{
	while (1) {
		...
		c = getopt_long(argc, argv, "D:s:d:b:i:o:lHOLC3NR24p:vS:I:m:e:t:f:",
				lopts, NULL);
		//printf("optind: %d\n", optind);
		//printf("optarg: %s\n", optarg);
		//printf("option: %c\n", c);
...
static void transfer_read_write(int fd)
{
	...
	if (rw_mode == 2) {
		snprintf(str, sizeof(str), "write");
	} else if (rw_mode == 3) {
		snprintf(str, sizeof(str), "read and write");
	} else {
		rw_mode = 1;
		snprintf(str, sizeof(str), "read");
	}

	srand(time(NULL));
	payload_len = random() % 990;
