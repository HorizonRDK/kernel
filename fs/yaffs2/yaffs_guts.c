...
void yaffs_set_obj_name(struct yaffs_obj *obj, const YCHAR * name)
{
	...
	if (name && !name[0]) {
		...
		YAFFS_SHORT_NAME_LENGTH)  {
		strncpy(obj->short_name, name, strlen(name));
	}
					...
					int old_chunk, u8 *buffer)
{
	...
	if (object && !yaffs_skip_verification(dev)) {
		if (tags.chunk_id == 0) {
			...
			matching_chunk = yaffs_find_chunk_in_file(object, tags.chunk_id,
			    										NULL);
		}
...
static int yaffs_check_gc(struct yaffs_dev *dev, int background)
{
	...
	do {
		...
		if (dev->n_erased_blocks < min_erased) {aggressive = 1;
			} else {
			if (!background
		    ...
		    int is_shrink, int shadows, struct yaffs_xattr_mod *xmod)
{
	...
	strncpy(old_name, _Y("silly old name"), strlen(_Y("silly old name")));
				...
				int buffer_size)
{
	...
	if (strnlen(name, YAFFS_MAX_NAME_LENGTH) == 0) {
		...
		/* make up a name */
		strncpy(local_name, YAFFS_LOSTNFOUND_PREFIX, strlen(YAFFS_LOSTNFOUND_PREFIX));
		strncat(local_name, x, strlen(x));
...
int yaffs_get_obj_name(struct yaffs_obj *obj, YCHAR *name, int buffer_size)
{
	...
	if (obj->obj_id == YAFFS_OBJECTID_LOSTNFOUND) {
		...
		strncpy(name, obj->short_name, strlen(obj->short_name));
