				   ...
				   struct yaffs_obj *obj)
{
	...
	yaffs2_checkpt_obj_bit_assign(cp, CHECKPOINT_RENAME_ALLOWED_BITS,
								obj->rename_allowed);
	yaffs2_checkpt_obj_bit_assign(cp, CHECKPOINT_UNLINK_ALLOWED_BITS,
								obj->unlink_allowed);
				     ...
				     struct yaffs_checkpt_obj *cp)
{
	...
	if (obj->variant_type != cp_variant_type) {
		char tmp[128];
		char *tmp_ptr = tmp;
		tmp_ptr += snprintf(tmp_ptr, sizeof(tmp) - strlen(tmp),
							"Checkpoint read object %d type %d chunk %d ",
							cp->obj_id, cp_variant_type, cp->hdr_chunk);
		tmp_ptr += snprintf(tmp_ptr, sizeof(tmp) - strlen(tmp),
							"does not match existing object type %d",
							obj->variant_type);
		yaffs_trace(YAFFS_TRACE_ERROR, "%s", tmp);
		return 0;
	}
	...
	if (parent) {
		if (parent->variant_type != YAFFS_OBJECT_TYPE_DIRECTORY) {
			...
			tmp_ptr += snprintf(tmp_ptr, sizeof(tmp) - strlen(tmp),
								"chunk %d Parent type, %d, not directory",
	...
	obj->rename_allowed = yaffs2_checkpt_obj_bit_get(cp,
											CHECKPOINT_RENAME_ALLOWED_BITS);
	obj->unlink_allowed = yaffs2_checkpt_obj_bit_get(cp,
											CHECKPOINT_UNLINK_ALLOWED_BITS);
...
static void yaffs2_do_endian_tnode(struct yaffs_dev *dev,
	...
	if (!tags.chunk_used) {
		...
		*found_chunks = 1;
		...
		if (!in) {
			...
			char *tmp_ptr = tmp;
			tmp_ptr += snprintf(tmp_ptr, sizeof(tmp) - strlen(tmp),
								"yaffs tragedy: Could not make object for ");
			tmp_ptr += snprintf(tmp_ptr, sizeof(tmp) - strlen(tmp),
								"object  %d at chunk %d during scan",
								tags.obj_id, chunk);
			yaffs_trace(YAFFS_TRACE_ERROR, "%s", tmp);
			return YAFFS_FAIL;
		}
		...
		if (!in->valid && in->variant_type !=
			...
			char *tmp_ptr = tmp;
			tmp_ptr += snprintf(tmp_ptr, sizeof(tmp) - strlen(tmp),
								"yaffs tragedy: Bad type, %d != %d, ",
								oh ? oh->type : tags.extra_obj_type,
								in->variant_type);
			tmp_ptr += snprintf(tmp_ptr, sizeof(tmp) - strlen(tmp),
								"for object %d at chunk %d during scan",
								tags.obj_id, chunk);
			yaffs_trace(YAFFS_TRACE_ERROR, "%s", tmp);
		...
		if (!in->valid &&
			...
			if (parent &&
				...
				char *tmp_ptr = tmp;
				tmp_ptr += snprintf(tmp_ptr, sizeof(tmp) - strlen(tmp),
						"yaffs tragedy: attempting to use non-directory ");
				tmp_ptr += snprintf(tmp_ptr, sizeof(tmp) - strlen(tmp),
						"as a directory in scan. Put in lost+found.");
				yaffs_trace(YAFFS_TRACE_ERROR, "%s", tmp);
