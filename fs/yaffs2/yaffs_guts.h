...
#ifndef FS_YAFFS2_YAFFS_GUTS_H_
#define FS_YAFFS2_YAFFS_GUTS_H_
...
struct yaffs_block_info {
	...
	u32 block_state:4;	/* One of the above block states. */
						/* NB use unsigned because enum is */
						/* sometimes an int */
	...
	u32 chunk_error_strikes:3; /* How many times we've had ecc etc failures */
...
struct yaffs_param {
	...
	int max_objects;	/* Set to limit the number of objects created. */
						/* 0 = no limit. */
	...
	/* need it. yaffs direct uses it to implement the faster readdir. */
	...
	int disable_soft_del;	/* yaffs 1 only: */
							/* Set to disable the use of softdeletion. */
...
struct yaffs_dev {
	...
	struct yaffs_obj *unlinked_dir;	/* Directory where unlinked */
					 				/* and deleted files live. */
	struct yaffs_obj *del_dir;	/* Directory where deleted objects */
								/* are sent to disappear. */
...
/* yaffs_checkpt_obj holds the definition of */
/* an object as dumped by checkpointing. */
...
/* The CheckpointDevice structure holds the device information that */
/* changes at runtime and must be preserved over unmount/mount cycles. */
