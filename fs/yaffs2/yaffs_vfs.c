...
#endif
#define YAFFS_DUMP_PAGE_SIZE 8096
...
#define PAGE_CACHE_SIZE YAFFS_DUMP_PAGE_SIZE
...
/* Space holding and freeing is done to ensure */
/* we have space available for write_begin/end */
/* For now we just assume few parallel */
/* writes and check against a small number. */
			       ...
			       const char *options_str)
{
	...
	while (options_str && *options_str && !error) {
		...
		if (!strcmp(cur_opt, "no-inband-tags")) {
			options->inband_tags = 0;
...
static char *yaffs_dump_dev_part0(char *buf, struct yaffs_dev *dev)
{
	...
	buf += snprintf(buf, YAFFS_DUMP_PAGE_SIZE,
						"start_block.......... %d\n", param->start_block);
	buf += snprintf(buf, YAFFS_DUMP_PAGE_SIZE,
						"end_block............ %d\n", param->end_block);
	buf += snprintf(buf, YAFFS_DUMP_PAGE_SIZE,
