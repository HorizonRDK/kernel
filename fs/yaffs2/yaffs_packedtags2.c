...
static void yaffs_dump_tags2(const struct yaffs_ext_tags *t)
{
	...
	tmp_ptr += snprintf(tmp_ptr, sizeof(tmp) - strlen(tmp),
						"ext.tags eccres %d blkbad %d chused %d ",
						t->ecc_result, t->block_bad, t->chunk_used);
	tmp_ptr += snprintf(tmp_ptr, sizeof(tmp) - strlen(tmp),
						"obj %d chunk%d byte %d del %d ser %d seq %d",
						t->obj_id, t->chunk_id, t->n_bytes,
						t->is_deleted, t->serial_number, t->seq_number);
	yaffs_trace(YAFFS_TRACE_MTD, "%s", tmp);
}
