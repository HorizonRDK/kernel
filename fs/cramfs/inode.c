				 ...
				 int silent)
{
	...
	if (err || sbi->mtd_point_size != PAGE_SIZE) {
		pr_err("unable to get one page direct memory access to mtd:%s\n",
		       sb->s_mtd->name);
		pr_err("err: %d, mtd_point_size: %u, PAGE_SIZE: %lu\n",
		 err, (uint32_t) sbi->mtd_point_size, PAGE_SIZE);
		return err ? : -ENODATA;
	}
	...
	if (err || sbi->mtd_point_size != sbi->size) {
		pr_err("unable to get full image direct memory access to mtd:%s\n",
		       sb->s_mtd->name);
		return err ? : -ENODATA;
	}
